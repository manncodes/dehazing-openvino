...
class RESIDE_Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        clear = Image.open(clear_img_path)
...
class HAZE55_Dataset(data.Dataset):
    def __init__(self, path, train, size=crop_size, format=".png"):
        super(HAZE55_Dataset, self).__init__()
        self.size = size
        print("crop size", size)
        self.train = train
        self.format = format
...
ITS_test_loader = DataLoader(
    dataset=HAZE55_Dataset(path + "/Haze", train=False, size="whole img"),
    batch_size=1,
    shuffle=False,
)
...
#         path + "/RESIDE/SOTS/outdoor", train=False, size="whole img", format=".png"
#     ),
#     batch_size=1,
#     shuffle=False,
# )
...
if __name__ == "__main__":
    x, y = next(iter(ITS_test_loader))
    x = FF.to_pil_image(x[0])
    x.show()
